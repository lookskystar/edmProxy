...
import com.edmProxy.gui.MainFrame;
import com.edmProxy.gui.panel.proxy.ProxyAbstractTableModel;
...
public class ProxyManagePanel extends JPanel {
	...
	private ProxyPageing proxyServerPageing;
	...
	private int sumNumPage = 0;// 总数据

	private String[] arr;

	private MainFrame source;
	private ProxyAbstractTableModel proxyServerAbstractTableModel;
	private ProxyButtonColumn updateButtonColumn;
	private ProxyButtonColumn delButtonColumn;
	...
	public ProxyManagePanel(final MainFrame source) {
		this.source = source;
		...
		fistPageButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				proxyServerPageing = new ProxyPageing();
				list = new ArrayList<ProxyEntity>();
				list = proxyServerPageing.pageingBypageNowAndpageSize(1,
						numPageInt);
				createTable(list);
		...
		nextPageButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				proxyServerPageing = new ProxyPageing();
				list = new ArrayList<ProxyEntity>();
				list = proxyServerPageing.pageingBypageNowAndpageSize(
						countPage, numPageInt);
				createTable(list);

				if (countPage == sumPage) {
					countPage = sumPage;
					JOptionPane.showMessageDialog(ProxyManagePanel.this,
		...
		lastPageButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				proxyServerPageing = new ProxyPageing();
				list = new ArrayList<ProxyEntity>();
				list = proxyServerPageing.pageingBypageNowAndpageSize(sumPage,
						numPageInt);
				createTable(list);
				countPage = sumPage;
		...
		delSelectButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (!"".equals(selectIdValues)) {
					...
					int count = proxyDAO.deleteBatch(ids);
...
//					}
		...
		queryButton = new JButton("\u67E5\u8BE2",Constants.checkBtnIcon);
		...
		showDataButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				sumPage = proxyServerPageing.findTotalPage(numPageInt);
				sumNumPage = proxyServerPageing.getTotalRow();
				countPageLabel.setText(1 + "");
				System.out.println(sumPage);
				if (sumPage == 0) {
					sumPageLabel.setText(1 + "");
				} else {
					sumPageLabel.setText(sumPage + "");
				}
				sumNumPageLabel.setText(sumNumPage + "");
	...
	public void createTable(ArrayList<ProxyEntity> list) {
		table = new JTable();
		...
		proxyServerAbstractTableModel = new ProxyAbstractTableModel(list);
		proxyServerAbstractTableModel.dataToTable();
		tableMainPanel.setLayout(null);
		table.setModel(proxyServerAbstractTableModel);
		updateButtonColumn = new ProxyButtonColumn(table, 11);
		// delButtonColumn=new ProxyServerButtonColumn(table, 12);
		// 获得表格的表格列类
		tableColumn = table.getColumnModel().getColumn(12);
		// 实例化JCheckBox
