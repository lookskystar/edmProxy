...
//		TABLENAME=readPrpperties.getTABLENAME();
	...
	public ArrayList<SendTaskEntity> getListByIds(String selectIdValues){
		System.out.println(selectIdValues);
		//sendTaskIdArr=sendTaskStrs.split(",");
		 list=new ArrayList<SendTaskEntity>();
		 StringBuffer sbSql = new StringBuffer();
		 try {
			conn = dbHelp.getConn();// 得到数据库连接
			sbSql.append("select * from sendTask_tab where id in(");
			sbSql.append(selectIdValues+")");
			System.out.println("----->"+sbSql.toString());
			stmt = conn.prepareStatement(sbSql.toString()); 
			rs = stmt.executeQuery(sbSql.toString()); // 执行sql取得结果集
			/* 循环将回复信息封装成List */
			while (rs.next()) {
				SendTaskEntity obj = new SendTaskEntity(); // 回复对象
				obj.setId(rs.getInt("id"));
				obj.setSendTask(rs.getString("sendTask"));
				obj.setPost(rs.getString("post"));
				obj.setTitle(rs.getString("title"));
				obj.setProxyStart(rs.getInt("proxyStart"));
				obj.setContentPath(rs.getString("contentPath"));
				obj.setAccessoryPath(rs.getString("accessoryPath"));
				obj.setSendTaskCount(rs.getInt("sendTaskCount"));
				obj.setLastsendTaskDate(rs.getDate("lastsendTaskDate"));
				obj.setAccountSendNum(rs.getInt("accountSendNum"));
				obj.setAccountStartLinks(rs.getInt("accountStartLinks"));
				obj.setSendTaskReceivesPath(rs.getString("sendTaskReceivesPath"));
				obj.setSendIntervalTime(rs.getInt("sendIntervalTime"));
	...
	/** 增加原始数据 */
	public int insert(SendTaskEntity obj) {
		int count = 0; // 接受返回,值
		String preparedSql = "insert into sendTask_tab"
				+ "(sendTask,post,title,proxyStart,contentPath,accessoryPath,"
				+ "sendTaskCount,accountSendNum,accountStartLinks,"
				+ "sendTaskReceivesPath,sendIntervalTime) "
				+ "values(?,?,?,?,?" +",?,?,?,?,?,?)";
		// 占位符号数组
		Object[] param = {obj.getSendTask(),obj.getPost(),
						  obj.getTitle(),obj.getProxyStart(),obj.getContentPath(),
						  obj.getAccessoryPath(),
						  0,obj.getAccountSendNum(),obj.getAccountStartLinks(),
						  obj.getSendTaskReceivesPath(),obj.getSendIntervalTime()};
...
//					"where receive=?);";
	...
	public ArrayList<SendTaskEntity> pageingBypageNowAndpageSize(int pageNow,int pageSize) {
		ArrayList<SendTaskEntity> list = new ArrayList(); // 用来保存对象列表
		//rowNew当前页，数据总量
		...
		try {
			...
			sbSql.append("select * from sendTask_tab limit ");
...
//					ResultSet.CONCUR_READ_ONLY);
	...
	//通过主机查询
	public SendTaskEntity findBySendTask(String sendTask) {
		SendTaskEntity obj = new SendTaskEntity(); // 回复对象
		try {
			conn = dbHelp.getConn(); // 得到数据库连接
			// SQL语句
			String preparedSql = "select * from " + "sendTask_tab where sendTask='"+sendTask+"'";
...
//	//条件和全部查询------------------------------------------------------------分页--------开结束
	...
	public int deleteBatch(String ids) {
		int count = 0; // 接受返回值
		// SQL语句
		String preparedSql = "delete from sendTask_tab where id in (?)";
		// 占位符号数组
